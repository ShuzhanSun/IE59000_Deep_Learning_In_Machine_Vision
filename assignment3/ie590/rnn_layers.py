...
def rnn_step_forward(x, prev_h, Wx, Wh, b, use_tanh = True):
    ...
    # If use_tanh is true, use np.tanh function, else use sigmoid function       #
    ##############################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    if(use_tanh):
        pass ## Write your code here. Use np.tanh
        h_raw = x.dot(Wx) + prev_h.dot(Wh) + b
        next_h = np.tanh(h_raw)
        cache = (x, prev_h, Wx, Wh, h_raw, next_h)
    else:
        pass ## Write your code here. Use sigmoid function defined in this file
        h_raw = x.dot(Wx) + prev_h.dot(Wh) + b
        next_h = sigmoid(h_raw)
        cache = (x, prev_h, Wx, Wh, h_raw, next_h)
...
def rnn_step_backward(dnext_h, cache, use_tanh = True):
    """
    ...
    # of the output value from tanh i.e. d_tanh = 1 - tanh ** 2.                 #
    # Note that derivate of sigmoid is d_sigmoid = sigmoid * (1-sigmoid)         #
    # If use_tanh is true, d_tan, else use d_sigmoid                             #
    ##############################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ...
    if(use_tanh):
        ...
        dh_raw = next_h * (1 - next_h) * dnext_h
    dWx = x.T.dot(dh_raw)
    dx = dh_raw.dot(Wx.T)
...
def rnn_forward(x, h0, Wx, Wh, b, use_tanh = True):
    """
    ...
    cache = {}
    for i in range(T):
        prev_h, cache_i = rnn_step_forward(x[:, i, :], prev_h, Wx, Wh, b, use_tanh)
        h[:, i, :] = prev_h
        cache[i] = cache_i
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ##############################################################################
...
def rnn_backward(dh, cache, use_tanh = True):
    """
    ...
    for i in reversed(range(T)):
        ...
        dx[:, i, :], dprev_h, dWx_tmp, dWh_tmp, db_tmp = rnn_step_backward(dnext_h, cache[i], use_tanh)
        dWx += dWx_tmp
        dWh += dWh_tmp
        db += db_tmp
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    dnext_c = dnext_h * o * (1 - np.tanh(next_c) ** 2) + dnext_c # Note: add dnext_c for top c pass
