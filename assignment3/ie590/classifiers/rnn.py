...
class CaptioningRNN(object):
    ...
    def __init__(self, word_to_idx, input_dim=512, wordvec_dim=128,
                 hidden_dim=128, cell_type='rnn', dtype=np.float32, use_tanh = True):
        """
    ...
    def loss(self, features, captions):
        ...
        if self.cell_type == 'rnn':
            hidden_rnn, cache_rnn = rnn_forward(captions_in_init, hidden_init, Wx, Wh, b, self.use_tanh)
        ...
        if self.cell_type == 'rnn':
            dcaptions_in_init, dhidden_init, grads['Wx'], grads['Wh'], grads['b'] = rnn_backward(dhidden_rnn, cache_rnn, self.use_tanh)
    ...
    def sample(self, features, max_length=30):
        ...
        for step in range(max_length):
            # (2)
            if self.cell_type == 'rnn':
                hidden_curr, _ = rnn_step_forward(word_embed, hidden_curr, Wx, Wh, b, self.use_tanh)
            else:
