...
def affine_forward(x, w, b):
    ...
    out = np.zeros([x.shape[0], w.shape[1]])  # (N, M)
    for i in range(x.shape[0]):
        out[i] = x[i].reshape(1,-1).dot(w) + b
...
def affine_backward(dout, cache):
    ...
    x_ND = x.reshape(x.shape[0], np.prod(x.shape[1:]))  # (N, D)
    dw = (x_ND.T).dot(dout)
...
def leaky_relu_backward(dout, cache):
    """
    Computes the backward pass for a layer of leaky rectified linear units (ReLUs).
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    pass # Write your code here
    xCoeff = np.ones_like(x)
    xCoeff[x<0] = xCoeff[x<0] * 0.01
    dx = xCoeff * dout
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ###########################################################################
    #                             END OF YOUR CODE                            #
...
def sigmoid_forward(x):
    """
    NEW
    Computes the forward pass for a layer of sigmoid nonlinear units.
    ...
    ###########################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    #TODO: Verify with the torch implementation. 
    out = 1 / (1 + np.exp(-1 * x))
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ###########################################################################
    #                             END OF YOUR CODE                            #
...
def sigmoid_backward(dout, cache):
    """
    NEW
    Computes the backward pass for a layer of sigmoid nonlinear units.
    ...
    ###########################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    #TODO: Verify it with torch implementation. 
    dx = out * (1 - out) * dout
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ###########################################################################
    #                             END OF YOUR CODE                            #
...
def batchnorm_backward_alt(dout, cache):
    ...
    dmu = np.sum(dx_hat/(-np.sqrt(var+eps)), axis=0) + dvar*np.mean((np.sum(-2*(x-mu), axis=0)))
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for i in range(H_out):
        for j in range(W_out):
            ...
            for k in range(F):
                out[:, k, i, j] = np.sum(x_pad_masked * w[k, :, :, :], axis=(1,2,3)) + b[k]
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    N, C, H, W = x.shape
    out, cache = batchnorm_forward(x.transpose(0, 3, 2 ,1).reshape(N*W*H, C), gamma, beta, bn_param)
...
def spatial_batchnorm_backward(dout, cache):
    ...
    N, C, H, W = dout.shape
    dx, dgamma, dbeta = batchnorm_backward(dout.transpose(0, 3, 2, 1).reshape(N*W*H, C), cache)
